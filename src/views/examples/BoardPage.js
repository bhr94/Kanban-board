...
class BoardPage extends React.Component {
  ...
  renderDefaultView = () => {
    return (
      <div onClick={this.changeEditMode} className="boardTitle">
  ...
  componentDidMount() {
    this.props.loadCurrentBoardList(
  ...
  addCard = (i) => {
    if (this.state.cardTitle.length > 0) {
      ...
      fetch(URL + "/createCard", {
        ...
        .then((data) => {
          this.props.addCurrentBoardCard(
            data,
            this.props.currentBoard.lists[i].listid
          );
  ...
  render() {
    ...
    if (!this.props.isCurrentBoardListPending) {
      ...
      console.log(
        "render  this.state.lists " + JSON.stringify(this.state.lists)
      );
    }
      ...
      return (
        <>
          ...
          <DndProvider backend={HTML5Backend}>
            <DragDropContext
              onDragEnd={(result) => this.onDragEnd(result, this.state.lists)}
            >
                    ...
                    lists.map((list, i) => {
                      return (
                        <div className="list list-wrapper">
                          {this.state.listEdit.listEditMode &&
                          this.state.listEdit.listId === list.listid ? (
                            <div>
                              <input
                                type="text"
                              ...
                              <Button
                                ...
                                className="addCardButton"
                                onClick={() =>
                                  this.changeListEditMode(list.listid)
                                }
                              >
                                X
                              </Button>
                              <Button
                                ...
                                className="cancelButton"
                                onClick={() =>
                                  this.updateListTitle(list.listid)
                                }
                              >
                                ✔
                              </Button>
                                        ...
                                        {(provided, snapshot) => (
                                          ...
                                          >
                                            {this.state.editCardMode
                                              .isEditCardOpen &&
                                            this.state.editCardMode.cardId ===
                                              card.cardid ? (
                                              <div>
                                                <textarea
                                                  type="text"
                          ...
                          {/* ==== */}
                          {this.state.addCardMode.isAddCardOpen &&
                          this.state.addCardMode.listId === i ? (
                            <div>
                              <textarea
                                type="text"
                        ...
                        <form>
                          ...
                          >
                            <span class="placeholder">
                              <span class="icon-sm icon-add"></span>Add another
                              list
                            </span>
                          </a>
                          <input
...
const mapDispatchToProps = (dispatch) => {
  return {
    createList: (listTitle, boardId, listId) =>
      dispatch(createListAction(listTitle, boardId, listId)),
    createCard: (cardContent, listId, boardId, cardId) =>
      dispatch(createCardAction(cardContent, listId, boardId, cardId)),
    loadCards: (listId, boardId, idToken) =>
      dispatch(loadCardsAction(listId, boardId, idToken)),
    loadCurrentBoard: (boardId, token) =>
      dispatch(loadCurrentBoardAction(boardId, token)),
    loadCurrentBoardList: (boardId, idToken) =>
      dispatch(loadCurrentBoardListAction(boardId, idToken)),
    addCurrentBoardList: (list) => dispatch(addCurrentBoardlistAction(list)),
    addCurrentBoardCard: (data, listId) =>
      dispatch(addCurrentBoardCardAction(data, listId)),
    updateListTitle: (listId, data) =>
      dispatch(updateListTitleAction(listId, data)),
    updateCardContent: (cardId, data) =>
      dispatch(updateCardContentAction(cardId, data)),
    updateList: (data) => dispatch(updateListAction(data)),
    deleteCardFromList: (sourceListId, data) =>
      dispatch(deleteCardFromPreviousListAction(sourceListId, data)),
  };
};
