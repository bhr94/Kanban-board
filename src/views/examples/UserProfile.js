          ...
          <section className="section section-lg">
            <Container className="py-lg-md d-flex">
              <div className="col px-0">
                <main ref="main">
                  <div className="position-relative">{/* shape Hero */}</div>
                </main>
                ...
                ? boards.map((board, i) => {
                    return (
                      <Card
                        ...
                        key={i}
                        onClick={() => this.goToBoard(board.boardId)}
                      >
                        {board.boardTitle}
                      </Card>
                    );
...
const mapDispatchToProps = (dispatch) => {
  return {
    createBoard: (boardTitle, boardId) =>
      dispatch(createBoardAction(boardTitle, boardId)),
    loadBoards: (userId, idToken) =>
      dispatch(loadBoardsAction(userId, idToken)),
    loadCurrentBoardList: (boardId, idToken) =>
      dispatch(loadCurrentBoardListAction(boardId, idToken)),
    loadLists: (boardId, idToken) =>
      dispatch(loadListsAction(boardId, idToken)),
